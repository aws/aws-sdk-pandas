...
The AWS Data Wrangler has undergone a name change and is now known as the AWS SDK for pandas (awswrangler). While the name has been updated, the functionality and usage of the library remain unchanged. You can continue to install it using the familiar `pip install awswrangler`, and there's no need to modify your existing code. In addition to the name change, the library has been relocated from AWS Labs to the main AWS GitHub organization. Despite this move, you can still access the project using its old URLs, thanks to GitHub's redirect feature. However, it is recommended to update your bookmarks accordingly. Furthermore, the documentation has been migrated to [aws-sdk-pandas.readthedocs.io](https://aws-sdk-pandas.readthedocs.io). Even though there is a new location, existing bookmarks will be automatically redirected to the updated site. Feel free to explore the library with confidence in its continued functionality and support.

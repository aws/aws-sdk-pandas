...
def assert_account_id(session):
    account_id = (session.boto3_session.client(
...
def get_account_id_remote(primitives, account_id):
    account_id.value = (primitives.session.boto3_session.client(
        "sts").get_caller_identity().get("Account"))
...
def test_multiprocessing(default_session):
    ...
    proc = mp.Process(target=get_account_id_remote,
                      args=(primitives, account_id))
    proc.start()
    proc.join()

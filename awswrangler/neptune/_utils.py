...
def _build_gremlin_update(g: GraphTraversalSource, row: Dict) -> GraphTraversalSource:
    g = g.V(str(row['~id']))
...
def _build_gremlin_vertices(g: GraphTraversalSource, row: Dict) -> GraphTraversalSource:
    g = (g.V(str(row['~id'])).
        fold().
        coalesce(
        __.unfold(),
...
def _build_gremlin_edges(g: GraphTraversalSource, row: pd.Series) -> GraphTraversalSource:
    g = (g.V(str(row['~from'])).
        ...
        coalesce(
        __.unfold(),
        _build_gremlin_vertices(
            __, {"~id": row['~from'], "~label": "Vertex"})).
        addE(row['~label']).
        to(
        __.V(str(row['~to'])).
            fold().
...
def _build_gremlin_properties(g, row):

...
class Glue:
    ...
    def create_table(
        ...
        if file_format == "parquet":
            ...
            table_input = Glue.csv_table_definition(
                table, partition_cols_schema, schema, path, compression, extra_args=extra_args
            )
        else:
    ...
    def add_partitions(
        self, database, table, partition_paths, file_format, compression, extra_args=None
    ):
        ...
        for partition in partition_paths:
            if file_format == "parquet":
                ...
                partition_def = Glue.csv_partition_definition(
                    partition=partition, compression=compression, extra_args=extra_args
                )
            else:
                raise UnsupportedFileFormat(file_format)
    ...
    @staticmethod
    def _build_schema(
        dataframe, partition_cols, preserve_index, indexes_position, cast_columns=None
    ):
        ...
        partition_cols_schema_built = [
            (name, partition_cols_types[name]) for name in partition_cols
        ]
    ...
    def csv_table_definition(
        table, partition_cols_schema, schema, path, compression, extra_args=None
    ):
        ...
        if serde == "OpenCSVSerDe":
            ...
            refined_par_schema = [
                (name, dtype) if dtype in dtypes_allowed else (name, "string")

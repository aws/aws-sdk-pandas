...
def test_load_from_env_variable(wr: ModuleType) -> None:
    s3_endpoint_url = "s3.us-east-1.amazonaws.com"

    with patch.dict(os.environ, {"WR_S3_ENDPOINT_URL": s3_endpoint_url}):
        wr.config.reset()

        assert wr.config.s3_endpoint_url == s3_endpoint_url

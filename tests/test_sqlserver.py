...
def test_dfs_are_equal_for_different_chunksizes(sqlserver_table, chunksize):
    con = wr.sqlserver.connect(connection="aws-data-wrangler-sqlserver")
    df = pd.DataFrame({"c0": [i for i in range(64)], "c1": ["foo" for _ in range(64)]})
    wr.sqlserver.to_sql(df=df, con=con, schema="dbo", table=sqlserver_table, chunksize=chunksize)

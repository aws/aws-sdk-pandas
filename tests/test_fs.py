...
def test_read_full(path, mode):
    ...
    with open_s3_object(path, mode=mode, block_size=100, newline="\n") as s3obj:
        if mode == "r":
...
def test_read_chunked(path, mode, block_size):
    client_s3 = boto3.client("s3")
    path = f"{path}0.txt"
    bucket, key = wr._utils.parse_path(path)
    text = "0123456789"
    client_s3.put_object(Body=text, Bucket=bucket, Key=key)
    with open_s3_object(path, mode=mode, block_size=block_size, newline="\n") as s3obj:
        if mode == "r":
            for i in range(3):
...
def test_read_line(path, mode, block_size):
    client_s3 = boto3.client("s3")
    path = f"{path}0.txt"
    bucket, key = wr._utils.parse_path(path)
...
def test_write_full(path, mode):
    client_s3 = boto3.client("s3")
    path = f"{path}0.txt"
    bucket, key = wr._utils.parse_path(path)
    text = "ajdaebdiebdkibaekdbekfbksbfksebkfjebkfjbekjfbkjebfkebwkfbewkjfbkjwebf"
    with open_s3_object(path, mode=mode, block_size=5_242_880, newline="\n") as s3obj:
        if mode == "wb":
...
@pytest.mark.parametrize("mode", ["wb", "w"])
@pytest.mark.parametrize("block_size", [5_242_880, 10_000_000])
@pytest.mark.parametrize("data_size", [6_000_000, 10_000_000, 12_000_000])
def test_write_chunked(path, mode, block_size, data_size):
    client_s3 = boto3.client("s3")
    path = f"{path}0.txt"
    bucket, key = wr._utils.parse_path(path)
...
def test_additional_kwargs(path, kms_key_id, s3_additional_kwargs):
    if s3_additional_kwargs is not None and "SSEKMSKeyId" in s3_additional_kwargs:
        s3_additional_kwargs["SSEKMSKeyId"] = kms_key_id
    path = f"{path}0.txt"
    with open_s3_object(path, mode="w", block_size=10_000_000, s3_additional_kwargs=s3_additional_kwargs) as s3obj:
        s3obj.write("foo")
    with open_s3_object(path, mode="r", block_size=10_000_000, s3_additional_kwargs=s3_additional_kwargs) as s3obj:

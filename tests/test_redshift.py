...
def test_insert_with_column_names(redshift_table):
    ...
    with pytest.raises(redshift_connector.error.ProgrammingError):
        wr.mysql.to_sql(df=df, con=con, schema="test", table=redshift_table, mode="append", use_column_names=False)

    wr.mysql.to_sql(df=df, con=con, schema="test", table=redshift_table, mode="append", use_column_names=True)

    df2 = wr.mysql.read_sql_table(con=con, schema="test", table=redshift_table)
